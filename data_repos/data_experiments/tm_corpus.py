...
df_lists = d.groupby(['year'])['tokenized_text'].apply(join_token_lists).reset_index()
# print(df_lists)
processing.finish()
df_tokens = d.groupby(['year'])['tokenized_text'].apply(join_token_terms).reset_index()
df_pages = d.groupby(['year'])['page_number'].sum().reset_index()
# df_terms = d.groupby(['year'])['term'].apply(' '.join).reset_index()
df_counts = d.groupby(['year'])['tokenized_counts'].sum().reset_index()

dl = pd.merge(df_lists, df_tokens, on=['year'])
dx = pd.merge(dl, df_pages, on=['year'])
# dn = pd.merge(dx, df_pages, on=['year'])
final_df = pd.merge(dx, df_counts, on=['year'])
final_df.rename(columns={'tokenized_text_x': 'token_lists', 'tokenized_text_y': 'token_texts'}, inplace=True)
final_df.to_csv('congo_years_data_tm.csv')
