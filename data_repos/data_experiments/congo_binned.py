...
df = pd.read_csv('../notebooks/combined_all_ner_congo_data_binned.csv')
d = df
...
df_terms = d.groupby(['date', 'binned'])['term'].apply(' '.join).reset_index()
df_counts = d.groupby(['date', 'binned'])['word_counts'].sum().reset_index()

dl = pd.merge(df_lists, df_terms, on=['date', 'binned'])
dx = pd.merge(dl, df_tokens, on=['date', 'binned'])
dn = pd.merge(dx, df_pages, on=['date', 'binned'])
...
final_df.to_csv('combined_pages_date_congo_text_ner_binned.csv')
